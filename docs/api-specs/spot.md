...
- `name_ko`가 "임진각평화누리"처럼 여러 단어를 띄어쓰기 없이 붙여놓은 경우 전체 문자열로는 검색이 안 될 수 있어, **`K_HERITAGE` 카테고리에 한해서만** 전체 검색 실패 시 끝에서부터 한 글자씩 줄여가며 최대 4회 재시도한다(최소 3글자까지, #112).
- **K_HERITAGE 외 카테고리는 재시도하지 않고 전체 이름 정확 매칭만 시도한다.** "카페", "퀸즈파크" 같은 흔한 일반명사가 상호명에 섞인 경우, 이름을 잘라 재검색하면 완전히 무관한 이미지(예: 스코틀랜드 커피숍, 골프대회 영상)가 라이선스만 맞아 채택되는 오탐이 실제로 확인되어, 이 카테고리들은 안전하게 스킵하는 쪽을 택했다.
