...
@Tag(name = "Content Media", description = "작품 영상/OST 조회")
@RestController
@RequestMapping("/api/v1/contents/{contentId}")
...
public class ContentMediaController {
    ...
    public ResponseEntity<ApiResponse<List<ContentVideoResponse>>> videos(@PathVariable Long contentId) {
        return ResponseEntity.ok(ApiResponse.success("작품 영상 조회 성공", workMediaQueryService.listVideos(contentId)));
    }

    @Operation(summary = "작품 OST")
    @GetMapping("/tracks")
    public ResponseEntity<ApiResponse<List<ContentTrackResponse>>> tracks(@PathVariable Long contentId) {
        return ResponseEntity.ok(ApiResponse.success("작품 트랙 조회 성공", workMediaQueryService.listTracks(contentId)));
    }
}
